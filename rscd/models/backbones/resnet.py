...
if __name__ == "__main__":
    ...
    # 创建模型实例（选择 ResNet18 或 ResNet50）
    model = get_resnet18()  # 或者使用 get_resnet50_OS8()

    # 计算参数量
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)

    # 输入张量的尺寸: (batch_size, in_channels, height, width)
    ...
    gflops = flop_counter.total() / 1e9  # 转换为 GFLOPs
