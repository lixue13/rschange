...
class NdMamba2_2d_v1(BaseNdMamba2):
    def __init__(self, cin,  cout,mamba_dim, **mamba2_args):
        super().__init__(cin, cout, mamba_dim, **mamba2_args)
...
if __name__ == '__main__':
    ...
    # Adjust the size of y to match the size of x

    y = torch.randn(1, 64, 128, 128).to('cuda')  # Change the size of y to match x
    net2 = NdMamba2_2d_v1(64, 128, 64).cuda()
    # Set the model to evaluation mode
    # net2.eval()

    net2(y)
    # Use FlopCountAnalysis to calculate FLOPs
    flops = FlopCountAnalysis(net2, y)  # Pass both x and y as inputs

    print(flop_count_table(flops))

    # Output the total number of parameters
    print(f"Total parameters: {sum(p.numel() for p in net2.parameters())}")
