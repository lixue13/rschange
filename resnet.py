...
# 定义一个新的 BasicBlock，替换 ResNet18 中的卷积层
class MSDConvBasicBlock(nn.Module):
    expansion = 1  # 添加 expansion 属性
    def __init__(self, in_channels, out_channels, stride=1, downsample=None):
        super(MSDConvBasicBlock, self).__init__()
        # 使用 MSDConv_SSFC 替换 ResNet18 中的卷积层
        self.conv1 = MSDConv_SSFC(in_channels, out_channels, kernel_size=3, stride=stride, padding=1)
        self.bn1 = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = MSDConv_SSFC(out_channels, out_channels, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        return out
class MSDConvResNet18(nn.Module):
    def __init__(self, block, layers, num_classes=1000):
        super(MSDConvResNet18, self).__init__()
        self.inplanes = 64
        ...
        # 替换原有的 BasicBlock 为 MSDConvBasicBlock
    ...
    def forward(self, x):
        ...
        return x
def MSDConv_ResNet18():
    return MSDConvResNet18(MSDConvBasicBlock, [2, 2, 2, 2])

# 创建带有 MSDConv 的 ResNet18 模型
model = MSDConv_ResNet18()
