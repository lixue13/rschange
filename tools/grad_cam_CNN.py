...
def get_args():
    ...
    parser.add_argument("--output_dir", default='output/WUHUCD')
    parser.add_argument("--layer", default="model.net.decoderhead.TFAM1")
    return parser.parse_args()
test_ckpt_path = 'E:/Remote_CD/my_model/rschange-main_3080/work_dirs/CLCD_BS4_epoch200/LDNet/version_1_MFA_3407_WUHUCD/ckpts/test/test_change_f1/test_change_f1=0.8923-epoch=93.ckpt'
...
def main():
    ...
    if args.layer == None:
        raise NameError(
            "Please ensure the parameter '--layer' is not None!\n e.g. --layer=model.net.decoderhead.LHBlock2.mlp_l")

    cfg = Config.fromfile(args.config)

    model = myTrain.load_from_checkpoint(test_ckpt_path, cfg=cfg)
    model = model.to('cuda')
    print(model.net)
    ...
    gradcam_output_dir = os.path.join(base_dir, args.layer)
